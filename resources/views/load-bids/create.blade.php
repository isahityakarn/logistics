            ...
            <div class="col-md-8 mx-auto">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">Add New Logistics Job Price</h5>
                    </div>
                    ...
                    <div class="card-body">
                        <form action="{{ route('load-bids.store') }}" method="POST">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="logisticjob_id" class="form-label">Logistics Job <span
                                                class="text-danger">*</span></label>
                                        <select name="logisticjob_id" id="logisticjob_id"
                                            class="form-select @error('logisticjob_id') is-invalid @enderror" required>
                                            <option value="">Select Job</option>
                                            @foreach ($logisticsJobs as $job)
                                                <option value="{{ $job->id }}"
                                                    {{ old('logisticjob_id') == $job->id ? 'selected' : '' }}>
                                                    #{{ $job->id }} -
                                                    {{ Str::limit($job->job_description ?? 'No description', 50) }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="status" id="status"
                                                ...
                                                <option value="assigned"
                                                    {{ old('status') == 'assigned' ? 'selected' : '' }}>Assigned</option>
                                                <option value="in_progress"
                                                    {{ old('status') == 'in_progress' ? 'selected' : '' }}>In Progress
                                                </option>
                                                <option value="picked_up"
                                                    {{ old('status') == 'picked_up' ? 'selected' : '' }}>Picked Up</option>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-save"></i> Create Price
                                </button>
                            </div>
                        </form>
                    </div>
